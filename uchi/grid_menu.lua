...
local UchiGridItem = InputContainer:extend{
    entry = nil,
    width = nil,
    height = nil,
    menu = nil,
...
function UchiGridItem:onTapSelect(arg, ges)
...
function UchiGridItem:onHoldSelect(arg, ges)
...
local UchiGridMenu = Menu:extend{
    columns = 3,
    grid_rows = 3,
}

function UchiGridMenu:_recalculateDimen()
...
function UchiGridMenu:updateItems(select_number)
    self.layout = {}
    ...
    for row = 1, rows_per_page do
        ...
        for col = 1, self.columns do
            ...
            if entry then
                local grid_item = UchiGridItem:new{
                    entry = entry,
...
function UchiGridMenu:onMenuSelect(item)
    if item and item.callback then
        item.callback(item)
    end
end

function UchiGridMenu:onMenuHold(item)

...
local UchiListItem = InputContainer:extend{
    entry = nil,
    width = nil,
    height = nil,
    menu = nil,
...
function UchiListItem:init()
    ...
    if self.entry.cover_id and self.entry.cover_type then
        ...
        local local_path = Cache.coverPath(self.entry.cover_type, self.entry.cover_id)
    ...
    if self.entry.cover_type == "book" or self.entry.book ~= nil then
        ...
        if type(book) == "table" then
            ...
            if type(book.metadata) == "table" and type(book.metadata.authors) == "table" and #book.metadata.authors > 0 then
                local names = {}
                for _i, a in ipairs(book.metadata.authors) do
...
function UchiListItem:onHoldSelect(arg, ges)
...
local UchiListMenu = Menu:extend{
    item_height = 110,
}

function UchiListMenu:_recalculateDimen()
...
function UchiListMenu:updateItems(select_number)
    self.layout = {}
    ...
    for row = 1, rows_per_page do
        ...
        if entry then
            local list_item = UchiListItem:new{
                entry = entry,
...
function UchiListMenu:onMenuSelect(item)
    if item and item.callback then
        item.callback(item)
    end
end

function UchiListMenu:onMenuHold(item)
